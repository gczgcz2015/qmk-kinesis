...
- 29 颗 Plum Twist SK6812 逐键 RGB，GP1 数据线，支持 VialRGB
...
原拇指区矩阵 `R5` 在本分支停用，固件中为 `NO_PIN`。`R4` 从 `GP28` 改接
`GP0`，释放 `GP28/GP29` 给左侧 Joy-Con 的 X/Y 模拟输入，同时保留
`GP1` 驱动 29 颗 Plum Twist SK6812 数据链；灯珠直接使用 RP2040-Zero 的 3V3/GND
并联总线，不需要额外的逻辑电平转换器。

Joy-Con 默认映射为 `W/A/S/D`，按压为 `Space`，支持斜向组合；五个映射都可在
Vial 的 `Layer 4` 中修改，具体位置见 [docs/WIRING.md](docs/WIRING.md)。

逐键 RGB 可在 Vial 的 Lighting 页面调整灯效、颜色、亮度和速度。固件将最大亮度
限制为 `32/255`，避免 29 颗灯使板载 3.3 V 稳压器过载或过热。
