...
完整焊接图见 [wiring-layout.svg](wiring-layout.svg)。Plum Twist PCB 的焊盘定义与板载
二极管方向以[官方接线说明](https://ryanis.cool/cosmos/plum-twist/wiring)为准。

## 关键结论：使用 ROW2COL

Plum Twist 自带二极管，并且方向固定为 `ROW2COL`。固件已相应设置为：
...
不要外接 1N4148，也不要把固件改回 `COL2ROW`。从 PCB 的热插拔座/焊盘侧观察时，
把标为 `R` 的焊盘接到行线，把标为 `C` 的焊盘接到列线。RGB 的 `+ / - / I / O`
焊盘本次全部留空。
...
2. 每条行线之间、每条列线之间没有短路。
3. 未按键时行列不导通；按键后只在对应 R/C 交点通过板载二极管单向导通。
